...
const GameScreen: React.FC<GameScreenProps> = ({ config, onEndGame }) => {
  ...
  >(
    (config.participants as (Participant | Team)[]).map((p) => ({
      ...
      wordsAttempted: 0,
      wordsCorrect: 0,
      powerUps: { ...loadOwnedPowerUps(), ...(p.powerUps || {}) },
    })),
  );
  ...
  const handlePowerUp = (pu: PowerUp) => {
    ...
    if (count > 0) {
      ...
      if (typeof window !== "undefined") {
        const stored = loadOwnedPowerUps();
        const newCount = Math.max(0, (stored[pu.id] || 0) - 1);
        localStorage.setItem(
          "ownedPowerUps",
          JSON.stringify({ ...stored, [pu.id]: newCount }),
        );
      }
      applyPowerUpEffect(pu.id);
    } else if (spendCoins(pu.cost)) {
      applyPowerUpEffect(pu.id);
    }
  };
...
<img src="img/books.svg" alt="Book icon" className="w-10 h-10 mx-auto mb-4" />
          ...
          <div className="flex gap-4 justify-center mb-4">
              {powerUps.map((pu) => {
                ...
                const canAfford = count > 0 || coins >= pu.cost;
                return (
                  <button
                    key={pu.id}
