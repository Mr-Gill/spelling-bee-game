...
const PracticeScreen: React.FC<PracticeScreenProps> = ({ words, onBack }) => {
  const [currentWord, setCurrentWord] = React.useState<Word | null>(() => pickWord(words));
  ...
  const moveToNextWord = React.useCallback(() => {
    setCurrentWord(previous => pickWord(words, previous?.word));
    setAnswer('');
    setFeedback('');
      ...
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-4xl flex-col items-center justify-center text-center">
        <div className="mb-8 w-full rounded-3xl border border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-sm">
          ...
          <p className="mx-auto max-w-2xl text-lg text-white/90">
            Quick low-pressure spelling before the main game. No scores, no lives, just a few words to settle in.
          </p>
        </div>
